...
const createWindow = () => {
    ...
    ipcMain.handle("bills.edit-one-bill", async (event, item) => {
        return await workspace.billadmDao.updateOneBill(item.id, item.money, item.type, item.income, item.description, item.tags, item.creationTime);
    });
